...
public class Excel {
    ...
    public static void execute(){
        ...
        List<Account> accountList=Bank.getComptes();
        for (int i = 0 ; i < accountList.size() ; i++){
            Row row = sheet.createRow(i+1);
            row.createCell(0).setCellValue(accountList.get(i).getNumeroCompte());
            row.createCell(1).setCellValue(accountList.get(i).getNom());
            row.createCell(2).setCellValue(accountList.get(i).getTypeCompte());
            row.createCell(3).setCellValue(accountList.get(i).getSolde());
        }
    ...
    private static void generateHeader(Sheet sheet, Workbook workbook) {
        ...
        String[] headers = {"Numéro de compte" , "Nom du client" , "Type de compte" , "Solde"};
